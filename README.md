...
SpecToDelivery 是一个基于开放 [Agent Skills 规范](https://agentskills.io/specification)的软件项目交付 Skill。它可以从产品文档建立新项目，也可以先恢复已有仓库的真实状态再接着实施；如果当前只需要规格和交接材料，它只产出可实施的规格，不修改代码。
...
本仓库只维护一套平台中立的 `SKILL.md`、参考规则、模板和脚本。平台专属内容只作为可选适配层；例如 `agents/openai.yaml` 用于 OpenAI 产品的界面元数据，其他工具可以忽略。

| 平台 | 官方发现或导入入口 | 当前状态 |
|---|---|---|
| [Codex](https://learn.chatgpt.com/docs/build-skills) | `~/.agents/skills`、项目 `.agents/skills` | 主要开发与验证环境 |
| [Claude Code](https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview) | `~/.claude/skills`、项目 `.claude/skills` | 标准与结构兼容；待平台端到端验证 |
| [GitHub Copilot](https://docs.github.com/en/copilot/concepts/agents/about-agent-skills) | `~/.agents/skills`、`~/.copilot/skills`；项目 `.agents/skills`、`.github/skills` 或 `.claude/skills` | 标准与结构兼容；待平台端到端验证 |
| [Cursor](https://cursor.com/docs/skills) | `~/.agents/skills`、`~/.cursor/skills`；项目 `.agents/skills` 或 `.cursor/skills` | 标准与结构兼容；待平台端到端验证 |
| [Gemini CLI](https://geminicli.com/docs/cli/using-agent-skills/) | `~/.agents/skills`、`~/.gemini/skills`；项目 `.agents/skills` 或 `.gemini/skills` | 标准与结构兼容；待平台端到端验证 |
| [OpenCode](https://opencode.ai/docs/skills) | `~/.agents/skills`、`~/.config/opencode/skills`；项目 `.agents/skills`、`.opencode/skills` 或 `.claude/skills` | 标准与结构兼容；待平台端到端验证 |
| [TRAE](https://www.trae.ai/blog/trae_tutorial_0115) | [TRAE IDE 3.5.44+](https://www.trae.ai/ja/changelog) 可用项目 `.agents/skills`；也可在 Settings → Rule & Skills → Skills 中导入 | 标准与结构兼容；待平台端到端验证 |

这里的“兼容”表示平台官方支持 `SKILL.md`，并且本仓库没有把核心流程绑定到某个 Agent 的专属功能；不表示所有平台、版本和权限配置下的行为完全相同。只有完成该平台的安装发现、触发、引用加载、脚本执行、`PLAN.md` 更新、根级与子目录 `AGENTS.md` 读取以及完整交接流程后，才标记为端到端验证。

不同平台的能力不一致时，Skill 会先检查当前环境并调整执行方式，但不降低目标、验收标准、安全边界或证据要求：没有子 Agent 就顺序执行；没有 Python 3 就直接建立或审计治理文件，并把辅助脚本检查标记为 `MANUAL_REQUIRED`；缺少浏览器、网络、MCP 或外部权限时，只有能产生同等证据的替代方式才可以继续，否则保留为 `MANUAL_REQUIRED` 或 `BLOCKED_EXTERNAL`。缺少工具不能用文字声明代替证据。
